...
_int CMainQuest_NPC::Update_GameObject(_float _fDeltaTime)
{
	...
	if (nullptr == pPlayer)
		return GAMEOBJECT::ERR;
	CMainQuest* pMainQuest = (CMainQuest*)pManagemnet->Get_GameObject(pManagemnet->Get_CurrentSceneID(), L"Layer_MainQuest");
	...
	if (pManagemnet->Key_Pressing('G'))
	{
		...
		// NPC에게 말걸기
		if (fDist <= 5.f)
			pMainQuest->Set_StartQuest(true);
		else
			pMainQuest->Set_StartQuest(false);
	}
