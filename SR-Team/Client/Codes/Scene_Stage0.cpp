...
_int CScene_Stage0::Update_Scene(_float _fDeltaTime)
{
	...
	if (!m_bInit)
	{
		...
		CSoundManager::Get_Instance()->PlayBGM(L"bgm_town.mp3");
...
HRESULT CScene_Stage0::Setup_Layer_NPC(const wstring & LayerTag)
{
	...
	if (FAILED(pManagement->Add_GameObject_InLayer(SCENE_STAGE0, L"GameObject_MainQuestNPC", SCENE_STAGE0, LayerTag)))
		return E_FAIL;
	if (FAILED(pManagement->Add_GameObject_InLayer(SCENE_STAGE0, L"GameObject_RandomBoxNPC", SCENE_STAGE0, LayerTag)))
		return E_FAIL;
...
HRESULT CScene_Stage0::Respawn_Palyer()
{
	CManagement* pManagement = CManagement::Get_Instance();
	if (nullptr == pManagement)
		return E_FAIL;
