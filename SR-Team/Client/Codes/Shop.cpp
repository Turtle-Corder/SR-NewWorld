...
HRESULT CShop::Check_BuyItem()
{
	...
	for (_int j = m_iStartIndex; j < 3 + m_iStartIndex; ++j)
	{
		for (_int i = 0; i < 4; ++i)
		{
...
HRESULT CShop::Add_Component_ShopItem()
{
	...
	for (_uint i = 0; i < 18; ++i)
	{
		...
		else if (i == 16)
		{
			...
			wsprintf(szItemTextureName, L"Component_Texture_Item_ForgottenNecklace");
		}
		else if (i == 17)
		{
			swprintf(pItem->szItemTag, sizeof(pItem->szItemTag) / sizeof(TCHAR),
				L"%s", L"LethalCape");
			wsprintf(szItemTextureName, L"Component_Texture_Item_LethalCape");
		}
...
HRESULT CShop::Render_ShopItem()
{
	...
	_int iIndex = 0;
	for (_int j = m_iStartIndex, k = 0; j < 3 + m_iStartIndex; ++j, ++k)
	{
		for (_uint i = 0; i < 4; ++i)
		{
			iIndex = j * 4 + i;
